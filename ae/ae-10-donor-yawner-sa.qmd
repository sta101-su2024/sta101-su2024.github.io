...
> `null_dist_donor` has `r nrow(null_dist_donor)` rows and `r ncol(null_dist_donor)` columns. Each row is a replicate, and `replicate` column indicates the replicate number and `stat` is the simulated proportion, $\hat{p}$.
