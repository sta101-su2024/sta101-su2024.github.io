...
Use the following visualizations for Question \_\_ through \_\_.
:::
...
blizzard_salary <- blizzard_salary |>
  mutate(
    ...
    performance_rating = case_when(
      performance_rating == "Developing" ~ 1,
      performance_rating == "Successful" ~ 2,
      performance_rating == "High" ~ 3,
      performance_rating == "Top" ~ 4,
      TRUE ~ NA_real_
    )
  ) |>
...
-   `performance_rating`: Most recent review performance rating, with levels `Developing`, `Successful`, `High`, and `Top`. The `Developing` level is the lowest rating and the `Top` level is the highest rating.
...
@fig-blizzard-hist-1 and @fig-blizzard-hist-2 show the distributions of annual salaries of hourly and salaried workers. The two figures show the same data, with the facets organized across rows and across columns.

```{r}
#| label: fig-blizzard-hist-1
#| fig-cap: Distribution of annual salaries of Blizzard employees, Option 1
#| fig-asp: 0.8
...
#| fig-cap: Distribution of annual salaries of Blizzard employees, Option 2
#| fig-asp: 0.4
#| fig-width: 10
...
a.  @fig-blizzard-hist-1
b.  @fig-blizzard-hist-2
...
When your teammate sees this model output, they remark "The coefficient for `performance_rating` is negative, that's weird. I guess it means that people who get higher performance ratings get lower raises." Which of the following is the best response to your teammate?



    ```{r}
    ggplot(blizzard_salary, aes(x = salary_type, fill = performance_rating)) +
      geom_bar()
