...
11. There are probably some `NA`s that are not being represented in the plot.
12. Mosaic plot. It shows the marginal distribution of salary type (proportion of hourly and salaried employees).
13. c - Option 3. Parallel lines and salaried line has a higher intercept since Hourly is the reference level in `raise_3_fit` and the slope for `salary_typeSalaried` is positive.
14. A parsimonious model is the simplest model with the best predictive performance.
15. Answers may vary.
