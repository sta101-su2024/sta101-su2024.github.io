...
::: callout-warning
THIS IS A DRAFT LAB, FINAL VERSION WILL BE POSTED BY FRIDAY!
:::
...
::: exercise
Do female lemurs differ in weight from male lemurs? Report the difference in mean weights between groups. Next, construct a 99% bootstrap distribution for the difference in means between groups and interpret it. If the interval covers 0, you might think there is no difference between groups. Does the interval cover 0?
:::

## Exercise 6

::: exercise
Create a new column that tells you whether or not a lemur was born in the first or second half of the year (January through June vs July through December). Create a meaningful plot to illustrate the weights of each group of lemurs. Use `theme_bw()` to replace the default gray plot background. Based on your figure, which group of lemurs weighs more? Is the distribution of one or both groups skewed or symmetric? See [here](https://openintro-ims2.netlify.app/explore-numerical.html?q=skew#boxplots) for more information about skew vs symmetric distributions.
:::
...
::: exercise
IMS - [Chapter 12 exercises](https://openintro-ims2.netlify.app/12-foundations-bootstrapping#chp12-exercises), #4: Bootstrap distributions of $\hat{p}$, III.
:::
