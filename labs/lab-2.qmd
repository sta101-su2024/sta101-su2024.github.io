...
::: exercise
How many observations are there in the courage data frame? How many variables? What does each observation in the `courage` data frame represent? Write your answer as narrative under \"Exercise 1\", using inline code chunks.
:::

## Exercise 2

::: exercise
Create a bar plot of Courage wins, losses, and ties (`result`). Additionally, calculate the numbers of wins, losses, and ties using the `count()` function. Summarize your findings in a sentence.
:::

## Exercise 3

::: exercise
Create a new variable indicating whether the game way played at home or away for The Courage. This variable should be called `home_courage` and take the value \"home\" if Courage is the home team and \"away\" if Courage is the away team. Then, with `count()`, determine how many home and how many away games the Courage played, and summarize your findings in a sentence.
:::
...
::: exercise
a. Create a visualization that displays the relationship between `home_courage` and `result`. 
b. Calculate the proportions of home and away games that the Courage won. 
c. Do your findings suggest a home-field advantage? Why or why not?
:::
...
::: exercise
How many points do the Courage score when they win (on average)? Note this is **different than how many points** they \"win by\". How many points do the Courage score when they lose on average?
:::

To calculate this we first need to determine how many points The Courage scored in every game. We can use `if_else()` logic again to find this value for each game, and store it in a new column, `courage_pts`.
...
::: exercise
Create a scatter plot of `opponent_pts` (y) vs `courage_pts` (x). Color the scatter plot by whether the Courage are `home` or `away`. Add `geom_jitter` as well as the line $y = x$ using the code below. Finally, `facet_wrap()` your plot by season. What does the line represent? What does it mean for a point to fall above the line? Below the line?
:::
...
**TO DO:** Add up points to confirm they sum to 50.
