...
> $H_0$ Issue of choice and party affiliation are independent
>
> $H_A$ Issue of choice and party affiliation are not independent
...
sum(null_dist$stat >= obs_stat)/1000
...
With a p-value close to 0, which is smaller than 0.05 discernability level, we reject the null. The data provides evidence that there is a relationship between party affiliation and issues voters want candidates to discuss.
...
1 - pchisq(obs_stat|> pull(stat), df = 18)
