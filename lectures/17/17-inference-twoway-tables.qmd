...
The expected value of a cell of column issue i row party p is: 
$$\left(\frac{\text{column i total}}{\text{table total}}\right) (\text{row p total}).$$
...
For example, for democrat/economy cell, we would get $\frac{(94 - 116.04)^2}{116.04} = 4.19$.
Then, to find the chi-squared test statistic $X^2$, we would add the values each cell gives us.
...
Turns out, the chi-squared test statistic follows a Chi-squared distribution when the null hypothesis is true and 2 conditions are satisfied:
